...
export class FlowClient {
  ...
  async get<T>(path: string): Promise<T> {
    const res = await fetchWithRetry(`${FlowClient.BASE}${path}`, { headers: await this.authHeaders() });
    if (!res.ok) throw new Error(`GET ${path} failed (${res.status}): ${await parseErrorBody(res)}`);
    return res.json() as Promise<T>;
  }

  async post<T>(path: string, body: unknown = {}): Promise<T> {
    const res = await fetchWithRetry(`${FlowClient.BASE}${path}`, {
      method: "POST",
  ...
  async getAll<T>(path: string): Promise<T[]> {
    const results: T[] = [];
    let nextUrl: string | null = `${FlowClient.BASE}${path}`;
    while (nextUrl) {
